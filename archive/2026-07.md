...
- `refactor(sync-status): share status state with sync manager` — `SyncStatusService` now owns the plugin-wide status snapshot and subscriptions; `SyncManager.updateMetadata()` marks tracked rows synced, covering Ribbon and context-menu pushes as well as panel actions (commit 9cefb25)
- `test(sync): guard never-pushed rename status` — regression test ensures a rename with no sync metadata remains Local only and has no `movedFrom` marker (commit e586fc2)
- `fix(sync): honor ignore patterns for direct push` — direct Ribbon/context-menu/command Push stops before local or remote access for a user-ignored path; batch Push has the same guard and saving relevant settings refreshes the ignore matcher (commit 61ca728)
- `fix(sync): reconcile moves from legacy metadata` — out-of-band move reconciliation treats a missing legacy `lastKnownPath` as its metadata record key, preserving safe SHA-based move detection after restart (commit ed04867)
